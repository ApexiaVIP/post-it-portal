...
// Temporary diagnostic: returns intermediate state so we can identify why
// the no-filter code path returns advisers=[] in production. Delete once
// the underlying bug is found.
export async function businessTrackerByAdviserDebug(
  year: number,
  scope: Scope,
  filterAdviserIds?: number[] | null,
) {
  const allDeals = await fetchYearDeals(year);
  const advisers = await fetchActiveAdvisers();
  const weeksArr = (() => {
  ...
  const weekSet = new Set(weeksArr);
  ...
  let dealsSeen = 0, dealsKeptWeek = 0, dealsKeptAdv = 0;
  for (const d of allDeals) {
    dealsSeen++;
  ...
  const byAdviserSummary: { id: number; weekCount: number }[] = [];
  for (const [id, m] of byAdviser) byAdviserSummary.push({ id, weekCount: m.size });

  // Run the actual function for comparison
  const real = await businessTrackerByAdviser(year, scope, filterAdviserIds);

  return {
    filterAdviserIds,
    wantedAdvisersKind: wantedAdvisers === null ? "null" : "Set",
    wantedAdvisersValues: wantedAdvisers ? Array.from(wantedAdvisers) : null,
    counts: {
      allDealsLen: allDeals.length,
      advisersLen: advisers.length,
      weeksArrLen: weeksArr.length,
      dealsSeen,
      dealsKeptWeek,
      dealsKeptAdv,
      byAdviserSize: byAdviser.size,
    },
    byAdviserSummary,
    advisersFromDb: advisers.map(a => ({ id: a.id, name: a.name })),
    realResult: {
      advisersCount: real.advisers.length,
      advisers: real.advisers.map(r => ({ id: r.adviser_id, name: r.adviser_name, weeksLen: r.weeks.length, total: r.total.total })),
      allAdvisersCount: real.allAdvisers.length,
    },
    sampleDeals: allDeals.slice(0, 3).map(d => ({
      adviser_id: d.adviser_id, week: d.week, status: d.status, commission: d.commission,
      weekType: typeof d.week, adviserIdType: typeof d.adviser_id,
    })),
  };
}
